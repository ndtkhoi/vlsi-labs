...
module ex4_T100_tb;
  ...
  `ifdef ANNOTATION
    initial
      $sdf_annotate("./03_outputs/ex4_delay_s1v2h.sdf",
                    DUT, , "annotate_ex4_T100.log", "MAXIMUM");
  `endif

  // Dump waveform
  initial begin
    $dumpfile("ex4_T100_wave.vcd");
    $dumpvars(0, ex4_T100_tb.DUT);
  end
  ...
  initial begin
    ...
    if (err_cnt == 0) begin
      $display("====================================");
      $display(" ex4_T100_tb : TEST PASSED (no error)");
      $display("====================================");
    end 
    else begin
      $display("====================================");
      $display(" ex4_T100_tb : TEST FAILED with %0d errors", err_cnt);
      $display("====================================");
    end
  ...
  //sau reset: moi chu ky Q tang 1
    ...
    if (!rst) begin
      if (Q !== expectedQ) begin
        err_cnt++;
        $display("[%0t] ERROR (T=100): cycle %0d: expected Q=%b, got Q=%b",
